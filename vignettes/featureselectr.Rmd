...
without forced stratification. Below is a plot of the cross-validation
folds without stratification (left) and after an update to the
original `fs_setup` object to include stratification (right):

```{r plot-cross, out.width = "100%"}
plot_cross(fs_setup) + plot_cross(fs_update)
```
