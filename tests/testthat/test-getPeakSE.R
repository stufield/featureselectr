...
# Testing ----
# Forward
test_that("`getPeakSE()` forward generates correct indices", {
  ...
  tabl1 <- lapply(vec, function(.x) rnorm(20, mean = .x)) |>
    data.frame() |> setNames(1:10)
  idx0 <- getPeakSE(tabl0)
  idx1 <- getPeakSE(tabl1)
...
# Backward
test_that("`getPeakSE()` backward generates correct indices", {
  tabl2 <- lapply(rev(vec), function(.x) rnorm(20, mean = .x)) |>
    data.frame() |> setNames(1:10)
  idx2 <- getPeakSE(tabl2, "backward")
