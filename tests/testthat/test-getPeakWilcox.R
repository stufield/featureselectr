...
# Testing ----
# Forward
test_that("`getPeakWilcox()` forward generates correct indices", {
  ...
  idx0 <- getPeakWilcox(tabl0)
  idx1 <- getPeakWilcox(tabl1)
...
# Backward
test_that("`getPeakWilcox()` backward generates correct indices", {
  tabl2 <- lapply(rev(vec), function(.x) rnorm(20, mean = .x)) |>
    data.frame() |> setNames(1:10)
  idx2 <- getPeakWilcox(tabl2, "backward")
